...
Эта документация описывает v2.0.0 — форк, переписанный под современный
формат плагина, модели Ollama (в частности `bge-m3`), блочную
гранулярность поиска и горячую перезагрузку. Для краткой английской
...
Сервер автоматически исключает сам `block_key` из результатов и не
возвращает блоки из той же заметки (чтобы не было «сам с собой»).
