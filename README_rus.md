...
**Что означает каждая env-переменная в шаблоне:**

| Переменная | Назначение | Когда менять |
|---|---|---|
| `SMART_VAULT_PATH` | Абсолютный путь к корню vault'а. **Обязательный.** | Всегда под конкретный vault. |
| `SMART_VAULT_NAME` | Метка vault'а для `meta.vault_name` в ответах. | Под конкретный vault — обычно равно имени папки. |
| `OLLAMA_HOST` | URL Ollama для семантического поиска. Опционально: если опустить, семантика пробуется через `host` из `embedding_models.ajson`, иначе сервер работает только в keyword-режиме. | Менять на порт/хост удалённого Ollama. Убрать, если семантика не нужна. |
| `OLLAMA_EMBED_MODEL` | Модель эмбеддингов в Ollama. | Если vault проиндексирован не `bge-m3:latest`, а, скажем, `mxbai-embed-large:latest` — укажи здесь. |
| `RRF_K` | Сглаживающая константа hybrid-RRF. Применяется одинаково к обоим спискам, **балансы semantic↔keyword не смещает**. | Редко. Больше `k` → меньше разница между соседними рангами. Дефолт 60 — индустриальный стандарт. |
| `RRF_SEMANTIC_WEIGHT` | Вес semantic-ранга в RRF-фузии. | Подними (0.8–0.9), если важна смысловая близость (запросы парафразами). |
| `RRF_KEYWORD_WEIGHT` | Вес keyword-ранга в RRF-фузии. | Подними (0.5+), если ищешь редкие имена/цитаты/аббревиатуры. Сумма с `RRF_SEMANTIC_WEIGHT` не обязана быть 1 — это просто относительные веса. |
| `MAX_NOTE_CONTENT_CHARS` (v2.2.2) | Cap на `get_note_content`. Дефолт 1 000 000 символов (~250-330k токенов русского). | Поднимай (до 50 000 000), если используешь модель с огромным контекстом. Опускай, если MCP-клиент жёстко режет большие ответы и хочется предсказуемый truncation. `full: true` в tool'е по-прежнему снимает cap полностью. |

**Опциональные env, которых нет в шаблоне, но есть в сервере**:

- `SMART_EMBED_MODEL_KEY` — явно указать активную модель эмбеддингов (полный ключ `provider#ts` или голый `model_key`), если autodetect резолвит не ту. Обычно не нужно.
- `DISABLE_SEMANTIC_SEARCH=1` — выключить Ollama даже если она доступна (форсирует keyword-режим).
- `DISABLE_WATCHER=1` — выключить файловый watcher (полезно на Dropbox/iCloud-томах, где `fs.watch` теряет события).

Эти три добавляй в `env` только при необходимости — дефолтное поведение для большинства сетапов корректное.
